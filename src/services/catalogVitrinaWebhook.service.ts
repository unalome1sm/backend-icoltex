...
/** Normaliza colorHEX SAP (sin #) a #RRGGBB. */
export function normalizeColorHex(raw: string | undefined): string | undefined {
  const t = raw?.trim();
...
function mapGroup(raw: CatalogVitrinaGroupRaw): CatalogVitrinaGroup | null {
  ...
  const desc = raw.descripciones?.[0];
  const descripcionCorta = optionalText(desc?.corta);
  const descripcionLarga = optionalText(desc?.larga);
  const caracteristicas = optionalText(raw.caracteristicas);
  const usos = optionalText(raw.usos);
  const cuidados = optionalText(raw.cuidados);
